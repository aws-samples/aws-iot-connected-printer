...
def createkeysandcertificate_execution_accepted(response):
    ...
    try:
        ...
        if (is_ci == False):
            ...
            ### do the thing here -- make it write the pem and private key to local file

            with open('/home/pi/.ssh/perm_cert.pem', 'w') as perm_cert:
                # print(createKeysAndCertificateResponse.certificate_pem)
                perm_cert.write(createKeysAndCertificateResponse.certificate_pem)
            
            with open('/home/pi/.ssh/perm_cert_pk.key', 'w') as perm_cert:
                # print(createKeysAndCertificateResponse.certificate_pem)
                perm_cert.write(createKeysAndCertificateResponse.private_key)
